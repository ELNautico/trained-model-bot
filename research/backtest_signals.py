...
# -----------------------------------------------------------------------------
# Run naming + IO helpers (self-contained)
# -----------------------------------------------------------------------------
def _utc_ts() -> str:
...
def compute_levels(entry_px: float, atr: float, cfg: SignalConfig) -> Tuple[float, float]:
    ...
    if not np.isfinite(atr) or atr <= 0:
        atr = max(0.005 * float(entry_px), 1e-6)
...
def position_size_shares(equity: float, entry_px: float, stop_px: float, cfg: SignalConfig, risk_per_trade: float) -> int:
    stop_dist = float(entry_px - stop_px)
    if stop_dist <= 0:
        return 0
        ...
        if shares > 0:
            ...
            if shares > 0 and exit_reason is None and model_exit:
                ...
                if X_now is not None and model is not None:
                    ...
                    stop_today, _ = compute_levels(float(closes[i]), float(atrs[i]), cfg)
...
def summarize(
    ...
    *,
    ...
    if label != "FULL":
        print(f"Window: {str(metrics_start)} .. {str(metrics_end)}")
...
def main() -> None:
    ...
    p.add_argument("--metrics-start", type=str, default=None)
    p.add_argument("--metrics-end", type=str, default=None)

    p.add_argument("--tag", type=str, default=None)
    p.add_argument("--out-dir", type=str, default="backtests", help="Output directory (default: backtests)")
    p.add_argument("--no-legacy-flat", action="store_true", help="Do NOT write legacy flat trades/equity CSVs")
    ...
    run_id = build_run_id(ticker=args.ticker, tag=args.tag, kind="backtest")
    rdir = run_dir(backtests_dir, run_id)
    rdir.mkdir(parents=True, exist_ok=True)

    trades_path_struct = rdir / "trades.csv"
    equity_path_struct = rdir / "equity.csv"
    trades_df.to_csv(trades_path_struct, index=False)
    equity_df.to_csv(equity_path_struct, index=False)
    ...
    print(f"Saved trades (structured): {trades_path_struct}")
    print(f"Saved equity (structured): {equity_path_struct}")

    # LEGACY flat output (default ON)
    if not args.no_legacy_flat:
        tkr = args.ticker.upper()
        tag = f"_{args.tag}" if args.tag else ""
        legacy_trades = backtests_dir / f"{tkr}_trades{tag}.csv"
        legacy_equity = backtests_dir / f"{tkr}_equity{tag}.csv"
        trades_df.to_csv(legacy_trades, index=False)
        equity_df.to_csv(legacy_equity, index=False)
        print(f"Saved trades (legacy flat): {legacy_trades}")
        print(f"Saved equity (legacy flat): {legacy_equity}")
    ...
    if ms is not None or me is not None:
        summarize(trades_df, equity_df, initial_cash=float(args.initial_cash), label="HOLDOUT", metrics_start=ms, metrics_end=me)
